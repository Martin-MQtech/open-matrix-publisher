# -*- coding: utf-8 -*-
"""百家号 (Baijiahao) 视频自动上传器

发布入口：https://baijiahao.baidu.com/builder/rc/edit?type=video
登录入口：https://baijiahao.baidu.com/
"""
...
class BaijiahaoUploader:
    ...
    async def upload(self, video_file: str, title: str, desc: str = "", tags: list[str] | None = None) -> dict:
        ...
        if not is_logged_in(PLATFORM, self.account_name):
            return make_result(False, "not_logged_in", "百家号未登录，请先扫码登录", platform=PLATFORM)

        print(f"[Baijiahao] 开始自动化分发: {v_path.name} | 标题: {title}")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                if not file_input:
                    await browser.close()
                    return make_result(False, "selector_error", "未找到百家号视频上传入口", platform=PLATFORM)

                await file_input.set_input_files(str(v_path))
                print("[Baijiahao] 视频文件已注入，等待上传解析...")
                await asyncio.sleep(8)

                # 填充标题
                title_input = await page.wait_for_selector("input[placeholder*='标题'], textarea[placeholder*='标题'], input.cheetah-input", timeout=15000)
                if title_input:
                ...
                # 等待并点击发布按钮
                publish_btn = await page.wait_for_selector("button:has-text('发布'), div:has-text('发布'):not(:has-text('发布中'))", timeout=20000)
                if publish_btn:
                    await publish_btn.click()
                    await asyncio.sleep(5)
                    print("[Baijiahao] 点击发布完成！")

                await browser.close()
                return make_result(True, "published", f"百家号发布成功: {title}", platform=PLATFORM)
            except Exception as e:
                await browser.close()
                return make_result(False, "upload_failed", f"百家号发布异常: {e}", platform=PLATFORM)
